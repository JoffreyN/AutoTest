...
def step1(args,**bro):#选择平台号
	...
	browser.get(f'{H5domainDic[args.env]}/subapps/bpep-credit-h5/index.html#/setupId')
	if saveToMysql:print(f"\n{envDict[args.env]} {time.strftime('%Y-%m-%d %X',time.localtime())}")
	else:save_file(f"\n{envDict[args.env]} {time.strftime('%Y-%m-%d %X',time.localtime())}")
	if args.robot:saveFileRobot(f"\n{envDict[args.env]} {time.strftime('%Y-%m-%d %X',time.localtime())}")
	# browser.find_element_by_css_selector('button.cube-btn.bg.cube-btn-primary').click()#点击开户注册
	browser=waitTo(browser,(SCENSEE,SCEWDE),way='css',name='span.cube-select-placeholder',operate='click')#点击所属平台
	time.sleep(0.5)
	#0:测试,1:甘肃,2:青海,3:安徽,4:湖南,5:重庆,6:江西,7:浙江,8:四川,9:海南,10:山东,11:湖北,12:北京
	# cube=0 if args.env=='c' else randint(0,11)
	# for i in range(cube+1):#选择平台号
	# 	browser.find_elements_by_css_selector('li.cube-picker-wheel-item')[i].click()
	# 	time.sleep(0.5)
	browser.find_element_by_css_selector('span.cube-picker-confirm').click()#点击确定
	time.sleep(0.5)
	#browser.find_element_by_css_selector('span.cube-picker-cancel').click()#点击取消
	# browser=waitTo(browser,SCEWDE,way='css',name='button.cube-btn.bg.cube-btn-primary',operate='click')#点击下一步
	return browser
...
def step3(args,browser):
	...
	browser=waitTo(browser,SCENSEE,way='id',name='id000',operate='click')
	time.sleep(0.5)
	if args.type=='4':
...
def fillCompanyInfo(args,browser):#types in [1,2,3,4]
	...
	# while 1:
	# 	browser.find_element_by_xpath('/html/body/div[1]/div/section/main/form/div[2]/div[2]/div').click()#所属行业
	# 	time.sleep(0.5)
	# 	try:
	# 		for i in range(1,randint(1,11)):#选择行业
	# 			browser.find_element_by_xpath(f'/html/body/div[2]/div[2]/div/div/div[2]/div/div/ul/li[{i}]').click()
	# 			time.sleep(0.5)			
	# 	except SCEENVE:continue
	# 	time.sleep(0.5)
	# 	try:
	# 		browser.find_element_by_xpath('/html/body/div[2]/div[2]/div/div/div[1]/span[2]').click()#点击确定
	# 		#browser.find_element_by_xpath('/html/body/div[2]/div[2]/div/div/div[1]/span[1]').click()#点击取消
	# 	except SCEENVE:continue
	# 	break
	time.sleep(0.5)
	browser.find_element_by_xpath('//*[@id="id3"]').click()#区域
	time.sleep(0.5)
	# for i in range(1,randint(1,32)):#选择区域
	# 	browser.find_element_by_xpath(f'/html/body/div[3]/div[2]/div/div/div[2]/div/div[1]/ul/li[{i}]').click()
	# 	time.sleep(0.5)
	...
	if args.type=='1':
		...
		for year in range(28,randint(20,25),-1):
			browser.find_element_by_xpath(f'/html/body/div[5]/div[2]/div/div/div[2]/div/div[1]/ul/li[{year}]').click()
			time.sleep(0.5)
		...
		time.sleep(0.5)
		browser.find_elements_by_class_name('cube-picker-confirm')[3].click()#点击确定
		#browser.find_element_by_xpath('/html/body/div[4]/div[2]/div/div/div[1]/span[1]').click()#点击取消
		time.sleep(0.5)
		#WebDriverWait(browser,10,0.5,WE).until(EC.element_to_be_clickable((By.XPATH,'/html/body/div[1]/div/section/main/form/div[6]/div[2]/div/label/input')))
...
def fillLegalInfo(args,browser,IDtypes=0):#IDtypes in [0,1]
	time.sleep(2)
	browser=waitTo(browser,SCEENIE,way='xpath',name='//*[@id="id1"]/div[2]/div/input',operate='send_keys',value=args.legalName)
	# if IDtypes:#选择回乡证
	# 	browser.find_element_by_xpath('//*[@id="id2"]/div[2]').click()#证件类型
	# 	time.sleep(0.5)
	# 	browser.find_element_by_xpath('/html/body/div[2]/div[2]/div/div/div[2]/div/div/ul/li[2]').click()#点击回乡证
	# 	time.sleep(0.5)
	# 	browser.find_element_by_class_name('cube-picker-confirm').clicl()#点击确定
	time.sleep(0.5)
	browser.find_element_by_xpath('//*[@id="id3"]/div[2]/div/input').send_keys(args.legalID)#证件号码
	browser.find_element_by_id('id11').click()#起始日期
	time.sleep(0.5)
	browser.find_elements_by_class_name('cube-picker-confirm')[1].click()#点击确定
	#browser.find_element_by_xpath('/html/body/div[3]/div[2]/div/div/div[1]/span[1]').click()#点击取消
	time.sleep(1)
	#WebDriverWait(browser,10,0.5,WE).until(EC.element_to_be_clickable((By.XPATH,'/html/body/div[1]/div/section/main/form/div[5]/div[2]/div/label/span[1]')))
...
def uploadPic(args,browser):
	...
	if args.type=='1':
		...
		time.sleep(1)
		# browser.find_element_by_xpath('/html/body/div/div/section/main/li[7]/div/li[2]/div/div[1]/div/input').send_keys(handIDpath)#手持身份证正面照
		time.sleep(1)
	elif args.type=='2':
		...
		time.sleep(1)
		# browser.find_element_by_xpath('/html/body/div/div/section/main/li[5]/div/li[2]/div/div[1]/div/input').send_keys(handIDpath)#手持身份证正面照
		time.sleep(1)
		...
		time.sleep(1)
		# browser.find_element_by_xpath('/html/body/div/div/section/main/li[4]/div/li[2]/div/div[1]/div/input').send_keys(handIDpath)#手持身份证正面照
		time.sleep(1)
	time.sleep(3)
	browser.find_element_by_tag_name('button').click()#提交
	if args.robot:saveFileRobot('上传图片完成')
	# WebDriverWait(browser,10,0.5,SCENSEE).until(EC.presence_of_element_located((By.TAG_NAME,'h3')))
	while 1:
		if '已提交' in browser.find_element_by_tag_name('h3').text:
			return browser
		else:
			try:
				browser.find_element_by_tag_name('button').click()#提交
...
def getParserH5():
	parser=argparse.ArgumentParser(description='程序功能：\n    H5注册+修改密码+预授信+申请白条',formatter_class=argparse.RawTextHelpFormatter)
	...
	parser.add_argument("--path",help="menhuPath",required=False,default='')
...
def main(args,**bro):
	try:
		...
		if args.env=='c':
			...
			out('正在获取初始密码...')
			password=getOMpassword(args.loginID,args.legalTel,env=args.env,path=args.path)
			out('正在获取商户号...')
			merchantNum=getMerchantNum(args.loginID, env=args.env,path=args.path)
			if saveToMysql:mysqlOpt(f"UPDATE registerinfo SET MerchantNum='{merchantNum}',InitialPWD='{password}' WHERE LoginID='{args.loginID}'")
		...
		if args.all in ['y',None,'Y']:
			browser=loginH5(username=args.loginID,passwd=newpassword,env=args.env,fillLoginID=0,menhuPath=args.path,bro=browser)#修改后的密码登陆
...
if __name__ == '__main__':
	...
	# args.legalName='杨光杰' if args.daddy=='2' else '林雪梅'
	args.doPrecredit=1
