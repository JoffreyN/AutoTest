...
from tools import out,goonORquit
from config import MMdomainDic
requests.packages.urllib3.disable_warnings()

def saveLog(Error,content):
	print(Error,'log路径：',os.path.join(os.getcwd(),f'{Error}.txt'))
	with open(f'{Error}.txt','a+',encoding='UTF-8') as log:
		log.write(f'{Error}:\n{content}\n')
...
def getProcessInstanceId(loginID,env='c',MMusername='',MMpassword=''):
	...
	head={
		...
		'Cookie':loginMMReq(env=env,username=MMusername,password=MMpassword)['Cookie'],
...
def fillCustomerInfo(processInstanceId,cusType,domain,head):#第二步填写客户信息
	out('正在填写客户信息……\t\t\t\t\t\t')
	soup=0
	for i in range(5):
...
def MMcreditElect(loginID,cusType,companyName,env='c',agreTempCode='',step='1'):#电子凭证授信
	if step=='6':
		result,liabHead=creditGrant(loginID,env)
		if result:
			# print('电子凭证授信完成！\t\t\t\t\t\t')
			return f'{loginID} 电子凭证授信完成!\t\t\t\t\t\t'
	else:
		domain,head,processInstanceId=getProcessInstanceId(loginID=loginID,env=env)#前奏 获取相关信息
		if step=='1':
			if signProduct(domain,head,processInstanceId,agreTempCode=agreTempCode):#第一步签约产品
				if fillCustomerInfo(processInstanceId,cusType,domain,head):#第二步填写客户信息
					if uploadCustomerPic(processInstanceId,head,domain):#第三步上传图片
						if bindBankCard(loginID,companyName,processInstanceId,domain,head):#第四步绑定银行卡
							if ICGR(processInstanceId,head,domain):#第五步录入授信结果
								...
								if result:
									# print('电子凭证授信完成！\t\t\t\t\t\t')
									return f'{loginID} 电子凭证授信完成!\t\t\t\t\t\t',head,liabHead
		elif step=='2':
			if fillCustomerInfo(processInstanceId,cusType,domain,head):
...
def addUser(company,head=''):#新增用户、角色
	if not head:
			head={
				'User-Agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.121 Safari/537.36',
				'Cookie':loginMMReq(env=company.env,username='',password='')['Cookie'],
				'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8',
				'Connection':'close',
			}
	key={
...
def getParserMMcreditElect():
	...
	parser.add_argument("-c",dest='cusType',help="要做进件的企业类型（默认 1）:\n    1: CORE 核心企业\n    2: CUSTOMER 供应商",required=False)
	parser.add_argument('-e',dest='env',help="运行环境（默认 c）：\n    k: 开发(46)环境\n    c: 测试环境\n    p: 准生产环境\n    s: 生产环境",required=False)
	# parser.add_argument("-u",dest='username',help="指定资金后台用户名（默认  ）",required=False)
	# parser.add_argument("-p",dest='password',help="指定资金后台密码（默认  ）",required=False)
	parser.add_argument("-s",dest='step',help="要从第几步开始做授信（默认 1）:\n    1: 签约产品\n    2: 填写客户信息\n    3: 上传图片\n    4: 绑定银行卡\n    5: 录入授信结果\n    6: 审核授信申请",required=False)
...
if __name__ == '__main__':
	...
	args.cusType=args.cusType if args.cusType else '1'
	args.step=args.step if args.step else '1'
	args.username=args.username if args.username else ''
	args.password=args.password if args.password else ''
	MMcreditElect(loginID=args.loginID,cusType=args.cusType,companyName=args.companyName,env=args.env,MMusername=args.username,MMpassword=args.password,agreTempCode='XYMB201903091129384912119',step=args.step)
	# print(getProcessInstanceId('pc201903081147',env='c',MMusername='businessLiable',MMpassword=''))